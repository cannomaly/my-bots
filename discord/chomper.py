...
# Bot token for Chomper from the environment file
TOKEN = os.getenv("DISCORD_TOKEN_CHOMPER")
print(f"Loaded Token: {TOKEN}")  # Check if the token is loaded correctly

if TOKEN is None:
    raise ValueError("DISCORD_TOKEN_CHOMPER is not set in the environment.")

# Set up intents (including message content intent)
intents = discord.Intents.default()
intents.messages = True  # Enable message event handling
intents.message_content = True  # Enable message content intent to receive message content
...
# Event to check if the bot is receiving any messages
@bot.event
async def on_message(message):
    print(f"Message from {message.author}: {message.content}")  # Debugging line to log incoming messages
...
async def help_command(ctx):
    """Display a list of all available commands for admins."""
    help_message = (
        "**Chomper Bot Commands:**\n"
        ...
        "\n**?urgent_announce message** - Send an urgent announcement with elevated priority."
